...
def get_random_seed():
    ...
    measurement_time = 15 # in seconds, How long to generate random bits for
    ...
    pulsed_measurement_logic.set_timer_interval(0.05) # set analysis timer interval to 50ms

    pulsed_master_logic.generate_predefined_sequence("rng2", kwarg_dict=dict(num_norm_pulses=50), sample_and_load = True)
    ...
    # print(f"Number of random bits: {len(pulsed_measurement_logic._diff_log)} in {pulsed_measurement_logic.elapsed_time:.1f} s")
    ...
    return "".join(str(int(x)) for x in rn_bits[:160])

    # print("Percentage of 0s and 1s:", percentages)

def get_random():
    pulsed_measurement_logic.raw_data_logging = True
    measurement_time = 1 # in seconds, How long to generate random bits for
    ...
    return "".join(str(int(x)) for x in rn_bits[:8])
